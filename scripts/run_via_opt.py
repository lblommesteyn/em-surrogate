"""Differential-via optimization application (frozen v2c methodology): hybrid surrogate + selective-solver vs
...
from emsurr import dataset, via_task as DT, models, splits, synth_ext
from emsurr.risk_cal import spearman
from emsurr.topo_rep import TopoEmbedding

STACK = Path("results/via_stack")
RUNS = Path("results/via_runs")   # per-run summaries
...
def train_stack():
    ...
    viamod = DT.make_via_model_family(400, seed=11)
    rs = np.random.RandomState(1)
    pool = train + gapped + viamod
    ...
    ens = []
    for i in range(5):
        p = STACK / f"ds6_{i}.pt"  # via-stack members
...
def run_multistart(solver, seed, gap_of, stats, freq, budget=BUDGET,
    ...
    def surr_eval(ds):
        ...
        return ([DT.surrogate_objective(mu[i], freq) for i in range(len(ds))], gap, unc)
...
def main():
    ...
    surr_J = [DT.surrogate_objective(mu[i], freq) for i in range(24)]
    ...
    for name, r in res["runs"].items():
        ...
        qa[name] = dict(design=d, verified_J=rr["J"], reported_J=r["best_J"],
                        surrogate_J=DT.surrogate_objective(mu[0], freq),
