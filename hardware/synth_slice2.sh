...
OUT=/root/jphys/out
mkdir -p $OUT
TAG=t1s${D}n${NE}bw${BW}
/root/oss-cad-suite/bin/yosys -q -p "
read_liberty -lib $RLIB;
read_verilog mac_array_p.v mac_slice2.v;
chparam -set D $D -set N_ENG $NE -set ACC $AC -set EXT_BW $BW mac_slice2;
hierarchy -top mac_slice2;
rename -top mac_slice2;
synth;
